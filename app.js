...
function tap(){ 
  ...
  g.gain.exponentialRampToValueAtTime(.0001,audioCtx.currentTime+.09); 
...
$("#refBtn").addEventListener("click",()=>{ 
  ...
  g.gain.setValueAtTime(.0001,audioCtx.currentTime);
  g.gain.exponentialRampToValueAtTime(.45,audioCtx.currentTime+.03);
  g.gain.setValueAtTime(.45,audioCtx.currentTime+.12);
  ...
  try{
    ...
    function tick(){ 
      ...
      if(freq>0){ 
        ...
        $("#status").textContent=Math.abs(cents)<6?"Afinado":cents<0?"Aperte a corda":"Afrouxe a corda"; 
      } 
...
function drawScore(k){ 
  ...
  notes.forEach((n,i)=>{ 
    const x=70+i*82, y=ymap[n]||ymap[n.replace("#","")]||120; 

    svg.innerHTML+=`<ellipse cx="${x}" cy="${y}" rx="18" ry="13" fill="white" transform="rotate(-12 ${x} ${y})"><animate attributeName="cy" values="${y};${y-5};${y}" dur="${2+i*.12}s" repeatCount="indefinite"/></ellipse><line x1="${x+16}" y1="${y}" x2="${x+16}" y2="${y-72}" stroke="white" stroke-width="3"/>`; 
...
function metroClick(){ 
  ...
  g.gain.setValueAtTime(.0001,audioCtx.currentTime); 
  g.gain.exponentialRampToValueAtTime(.32,audioCtx.currentTime+.006); 
